...
SECRET_PATTERNS = (
    ...
    re.compile(r"(?i)\b(AKI[AS][A-Z0-9]{16})\b"),
...
def _render(arg: str, *, job: BuilderJob, job_file: str) -> str:
    values = {
        "{task_id}": job.task_id, "{job_file}": job_file,
        "{working_directory}": job.working_directory, "{branch}": job.branch,
        "{baseline_commit}": job.baseline_commit, "{contract_path}": job.contract_path,
...
def _job_hash(job: BuilderJob) -> str:
    return hashlib.sha256(json.dumps({
        "task_id": job.task_id, "builder": job.builder, "repository": job.repository,
        "working_directory": job.working_directory, "branch": job.branch,
        "baseline_commit": job.baseline_commit, "contract_path": job.contract_path,
...
def dispatch(job: BuilderJob, spec: BuilderSpec, *, state_dir: str = "/var/lib/hermes-builder") -> dict[str, Any]:
    ...
    with lock_path.open("w") as lock_fp:
        ...
        job_payload = {
            ...
            "working_directory": job.working_directory, "branch": job.branch,
            "baseline_commit": job.baseline_commit, "contract_path": job.contract_path,
            "timeout_seconds": job.timeout_seconds, "job_sha256": _job_hash(job),
        }
        ...
        exit_code, stdout, stderr, timed_out = _run_builder(
            cmd, cwd=job.working_directory, env=env, timeout=job.timeout_seconds
        )
        return {
            "task_id": job.task_id, "builder": job.builder,
            "status": "COMPLETED" if exit_code == 0 else ("TIMEOUT" if timed_out else "FAILED"),
            "exit_code": exit_code, "timed_out": timed_out,
            "duration_seconds": round(time.time() - started, 3),
            "job_sha256": job_payload["job_sha256"],
            "stdout": redact(stdout[-12000:]), "stderr": redact(stderr[-12000:]),
            "command_executable": spec.executable, "branch": job.branch,
...
def dispatch_from_files(job_path: str, config_path: str, result_path: str, *, state_dir: str = "/var/lib/hermes-builder") -> int:
    ...
    out = Path(result_path); out.parent.mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(); parser.add_argument("job"); parser.add_argument("config"); parser.add_argument("result")
