...
def _git_env(key: Path) -> dict[str, str]:
    ...
    return {
        ...
        "SSH_AUTH_SOCK": "",
        "GIT_SSH_COMMAND": (
            f"/usr/bin/ssh -i {shlex.quote(str(key))} -o IdentitiesOnly=yes -o BatchMode=yes "
...
def _run_git(args: list[str], *, cwd: Path, key: Path, timeout: int = 180) -> subprocess.CompletedProcess:
    return subprocess.run(["git", "-c", "core.hooksPath=/dev/null"] + args, cwd=str(cwd), capture_output=True, text=True, timeout=timeout, shell=False, env=_git_env(key))
...
def _remote_snapshot(remote: str, protected: list[str], *, cwd: Path, key: Path) -> dict[str, str]:
    ...
    for branch in protected:
        ...
        line = r.stdout.strip().splitlines(); snap[branch] = line[0].split()[0] if line else ""
    return snap
...
def _prepare_clone(q: QueueJob, cfg: dict) -> tuple[Path, Path, Path, str, dict[str, str]]:
    ...
    remote = str(cfg["remote_url"]); key = _secure_key(str(cfg["ssh_key"]), q.repository)
    root = Path(str(cfg["clone_root"])).expanduser().resolve(); root.mkdir(parents=True, exist_ok=True)
    ...
    contract_root = (clone / str(cfg.get("contract_root_relpath", "docs/tasks"))).resolve(); contract = (contract_root / q.contract_relpath).resolve()
...
def _finalize_git(q: QueueJob, clone: Path, key: Path, remote: str, before: dict[str, str], result: dict) -> dict:
    ...
    allowed_files = [str(x) for x in gate.get("allowed_files", [])]; protected_paths = [str(x) for x in gate.get("protected_paths", [])]
    ...
    if _run_git(["status", "--porcelain"], cwd=clone, key=key).stdout.strip(): raise DispatchError("builder left uncommitted changes")
    ...
    bad_allowed = [p for p in changed if not _path_allowed(p, allowed_files)]; bad_protected = [p for p in changed if _path_protected(p, protected_paths)]
    ...
    result.update({"candidate_sha": candidate, "branch": q.branch, "pushed": True, "changed_files": changed, "protected_branch_snapshot": before}); return result
...
def _claim(task_id: str) -> bool:
    ...
    payload = {"task_id": task_id, "processor_id": PROCESSOR_ID, "claimed_at": datetime.now(timezone.utc).isoformat(), "claim_nonce": uuid.uuid4().hex}
    ok, _, _ = commit_and_push([(_claim_path(task_id), json.dumps(payload, indent=2))], f"builder-claim: {task_id} by {PROCESSOR_ID}"); return ok
...
def _publish(task_id: str, payload: dict, *, completed: bool) -> None:
    ...
    ok, msg, _ = commit_and_push([(target, json.dumps(payload, indent=2))], f"builder-completed: {task_id} status={payload.get('status')}", delete_paths=[f"{INBOX}/{task_id}.json"])
...
def process_once(config_path: str, *, state_dir: str) -> int:
    ...
    config = Path(config_path).expanduser(); load_config(str(config)); raw_cfg = json.loads(config.read_text()); specs = load_config(str(config)); processed = 0
    for inbox_file in sorted(inbox.glob("*.json")):
        ...
        try:
            ...
            cfg = _repo_config(raw_cfg, q.repository); clone, contract, key, remote, before = _prepare_clone(q, cfg)
            if not _claim(q.task_id): continue
            job = BuilderJob(q.task_id, q.builder, q.repository, str(clone), q.branch, q.baseline_commit, str(contract), q.timeout_seconds)
            result = dispatch(job, specs[q.builder], state_dir=state_dir); result.update({"hos_gate_task_id": q.hos_gate_task_id, "hos_gate_receipt": q.hos_gate_receipt})
            if result["status"] != "COMPLETED": raise DispatchError(f"builder {result['status']}: {result.get('stderr','')[-500:]}")
            result = _finalize_git(q, clone, key, remote, before, result); result["completed_at"] = datetime.now(timezone.utc).isoformat(); _publish(q.task_id, result, completed=True)
        except Exception as exc:
            task_id = q.task_id if q is not None and q.task_id else inbox_file.stem
            payload = {"task_id": task_id, "status": "STOPPED", "error": type(exc).__name__, "summary": redact(str(exc)), "completed_at": datetime.now(timezone.utc).isoformat()}
...
def main() -> int:
    import argparse
    parser = argparse.ArgumentParser(); parser.add_argument("--config", default=str(DEFAULT_ROOT / "config.json")); parser.add_argument("--state-dir", default=str(DEFAULT_ROOT / "state")); parser.add_argument("--poll", type=int, default=30); parser.add_argument("--once", action="store_true"); args = parser.parse_args()
