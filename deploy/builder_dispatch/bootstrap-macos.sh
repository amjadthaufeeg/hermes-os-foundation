...
KNOWN_HOSTS="$HOME/.ssh/hermes-builder-known_hosts"
...
if id -Gn | tr ' ' '\n' | grep -qx admin; then fail "The builder account must be NON-ADMIN."; fi
...
mkdir -p "$APP_ROOT" "$STATE_ROOT" "$LOG_ROOT" "$CLONE_ROOT" "$HOME/Library/LaunchAgents" "$HOME/.ssh"
chmod 700 "$APP_ROOT" "$STATE_ROOT" "$LOG_ROOT" "$CLONE_ROOT" "$HOME/.ssh"
touch "$KNOWN_HOSTS"; chmod 600 "$KNOWN_HOSTS"

ssh_env() {
  local key="$1"
  printf '%s' "ssh -i $key -o IdentitiesOnly=yes -o BatchMode=yes -o StrictHostKeyChecking=accept-new -o UserKnownHostsFile=$KNOWN_HOSTS"
}
verify_repo_key() {
  local repo="$1" key="$2"
  GIT_SSH_COMMAND="$(ssh_env "$key")" GIT_TERMINAL_PROMPT=0 git ls-remote "git@github.com:$repo.git" HEAD >/dev/null 2>&1 || fail "Dedicated key cannot access $repo"
}
...
if [[ ! -d "$FOUNDATION_REPO/.git" ]]; then
  ...
  GIT_SSH_COMMAND="$(ssh_env "$FOUNDATION_KEY")" GIT_TERMINAL_PROMPT=0 git clone "git@github.com:amjadthaufeeg/hermes-os-foundation.git" "$FOUNDATION_REPO" || fail "Cannot clone foundation repo"
fi
...
<plist version="1.0"><dict><key>Label</key><string>$LABEL</string><key>ProgramArguments</key><array><string>$PYTHON</string><string>-m</string><string>deploy.builder_dispatch.mac_worker</string><string>--config</string><string>$CONFIG</string><string>--state-dir</string><string>$STATE_ROOT</string><string>--poll</string><string>30</string></array><key>EnvironmentVariables</key><dict><key>PYTHONPATH</key><string>$SOURCE_ROOT</string><key>HOME</key><string>$HOME</string><key>PATH</key><string>$KIMI_DIR:/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin</string><key>HERMES_BUILDER_CONTROL_DIR</key><string>$APP_ROOT/hermes-control</string><key>HERMES_CONTROL_SSH_KEY</key><string>$CONTROL_KEY</string></dict><key>RunAtLoad</key><true/><key>KeepAlive</key><true/><key>ThrottleInterval</key><integer>10</integer><key>StandardOutPath</key><string>$LOG_ROOT/worker.out.log</string><key>StandardErrorPath</key><string>$LOG_ROOT/worker.err.log</string></dict></plist>
PLIST
chmod 600 "$PLIST"
...
/usr/bin/env -i HOME="$HOME" PATH="/usr/bin:/bin:/usr/local/bin:/opt/homebrew/bin" PYTHONPATH="$SOURCE_ROOT" HERMES_BUILDER_CONTROL_DIR="$APP_ROOT/hermes-control" HERMES_CONTROL_SSH_KEY="$CONTROL_KEY" "$PYTHON" -m deploy.builder_dispatch.mac_worker --config "$CONFIG" --state-dir "$STATE_ROOT" --once
...
JSON
/usr/bin/env -i HOME="$HOME" PATH="/usr/bin:/bin:/usr/local/bin:/opt/homebrew/bin" PYTHONPATH="$SOURCE_ROOT" HERMES_BUILDER_CONTROL_DIR="$APP_ROOT/hermes-control" HERMES_CONTROL_SSH_KEY="$CONTROL_KEY" "$PYTHON" - "$STATUS" "builders/worker-status/${HOST// /-}.json" <<'PY'
import sys
