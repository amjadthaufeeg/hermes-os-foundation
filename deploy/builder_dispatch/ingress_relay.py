...
def _target_for(path: str) -> tuple[str, str] | None:
    if path.startswith(TASK_PREFIX):
        ...
        if "/" in name or not NAME_RE.fullmatch(name): return None
...
def _validate(kind: str, filename: str, content: str) -> dict:
    raw = content.encode()
    if len(raw) > MAX_BYTES: raise ValueError("ingress payload too large")
    data = json.loads(content)
    if not isinstance(data, dict): raise ValueError("ingress payload must be object")
    ...
    if kind == "task":
        if str(data.get("schema_version", "")) != "1.0": raise ValueError("task schema_version must be 1.0")
        if not isinstance(data.get("contract"), dict) or not data["contract"]: raise ValueError("task contract required")
    else:
        for field in ("builder", "repository", "branch", "baseline_commit", "contract_relpath", "hos_gate_task_id", "hos_gate_receipt", "hos_gate_contract"):
            if not data.get(field): raise ValueError(f"builder job missing {field}")
    return data
...
def process_once() -> int:
    ...
    fetch = _run(["git", "fetch", "origin", f"{INGRESS_BRANCH}:refs/remotes/origin/{INGRESS_BRANCH}"], cwd=repo)
    if fetch.returncode != 0: return 0
    tree = _run(["git", "ls-tree", "-r", "--name-only", f"origin/{INGRESS_BRANCH}", "--", "ingress"], cwd=repo)
    if tree.returncode != 0: return 0

    state = _load_state(); processed = 0
    for path in sorted(x.strip() for x in tree.stdout.splitlines() if x.strip()):
        mapped = _target_for(path)
        if not mapped: continue
        kind, target = mapped
        show = _run(["git", "show", f"origin/{INGRESS_BRANCH}:{path}"], cwd=repo)
        if show.returncode != 0: continue
        digest = hashlib.sha256(show.stdout.encode()).hexdigest()
        if state.get(path) == digest: continue
        try:
        ...
        if target_path.exists():
            if hashlib.sha256(target_path.read_bytes()).hexdigest() == digest:
                state[path] = digest; processed += 1; continue
            continue

        # Never replay a completed/stopped task under the same id.
        task_id = Path(path).stem
        terminal_paths = [repo / "tasks" / "completed" / f"{task_id}.json",
                          repo / "builders" / "completed" / f"{task_id}.json",
                          repo / "builders" / "stopped" / f"{task_id}.json"]
        if any(p.exists() for p in terminal_paths):
            state[path] = digest; continue

        success, _, _ = commit_and_push([(target, show.stdout)], f"ingress-promote: {task_id}")
        if success:
            state[path] = digest; processed += 1
    _save_state(state)
    return processed
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(); parser.add_argument("--once", action="store_true"); parser.add_argument("--poll", type=int, default=20)
    args = parser.parse_args()
    if args.once: process_once(); return 0
    while True:
        process_once(); time.sleep(max(10, args.poll))
